...
//                   'Fornecedor', 'Tamanho','Cor' ,'QNT', 'QNT Produto', 'Custo', 'Preço à Prazo', 'Preço à Vista', 'Nº Parcelas', 'R$ Parcela', 'Validade']
    ...
    ItemEstoqueTableModel(produto) {
        super(produto
                ? ['ID', 'Nome', 'Tamanho', 'Cor', 'QNT', 'Preço a Prazo R$']
                : ['id', 'Código de Baras', 'Nome', 'Fornecedor', 'QNT', 'Custo', 'Preço à Prazo', 'Preço à Vista']);
    ...
    @Override
    void editar(Window parent, ItemEstoque ie) {
        new TelaItemEstoque(parent, ie).setVisible(true)
    }

    @Override
    void remover(Window parent, ItemEstoque ie) {
    ...
    @Override
    protected getValorColuna(String coluna, ItemEstoque ie) {
        switch (coluna) {
